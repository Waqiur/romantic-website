...
const AppProjects = () => {
  ...
  const handleProjectClick = (projectId: number) => {
    startNavigation(`/app-project/${projectId}`);
  };
  ...
  const SimpleLoadingFallback = () => (
    <div className="fixed inset-0 bg-black/90 z-50 flex items-center justify-center animate-fadeIn">
      <div className="w-20 h-1 bg-purple-500/30 rounded-full overflow-hidden">
        <div className="h-full bg-purple-500 animate-loadingBar"></div>
      </div>
    </div>
  );
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-purple-950 to-pink-950">
      ...
      <NavBar />
      <div className="container mx-auto px-8 relative z-10 pt-28 pb-20">
        {/* Navigation back */}
        <div className="mb-10 mt-2 ml-6">
          <Link href="/#projects" className="inline-flex items-center gap-2 text-purple-300 hover:text-white transition-all text-lg">
        ...
        >
          <h1 className="text-4xl md:text-5xl font-bold font-inter mb-6 bg-gradient-to-r from-purple-400 to-pink-300 bg-clip-text text-transparent">
            App Development Projects
          </h1>
          <p className="text-gray-300 max-w-3xl mx-auto">
            Discover my mobile and app development projects featuring augmented
            reality, cross-platform compatibility, and beautiful UIs.
          </p>
        </motion.div>

        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 max-w-6xl mx-auto">
          {appProjects.map((project: Project, index: number) => (
            <motion.div
              key={project.id}
              className="card-3d rounded-xl overflow-hidden bg-black/40 shadow-xl backdrop-blur-sm border border-purple-900/60 cursor-pointer transition-all hover:scale-105 hover:shadow-purple-500/20 hover:border-purple-800/60"
            ...
            >
              <div className="card-inner relative flex flex-col h-full">
                <div className="absolute inset-0 bg-gradient-to-br from-purple-800/20 to-pink-800/10 z-0"></div>
                ...
                <div className="p-6 relative z-10 flex-1 flex flex-col">
                  ...
                  <div className="flex flex-wrap gap-2 mb-6">
                    {project.technologies.map((tech: string) => (
                      <span
                        key={tech}
                        className="inline-flex items-center px-3 py-1 bg-purple-800/30 text-purple-200 rounded-full text-xs font-medium border border-purple-700/30"
                      >
                        {getTechIcon(tech)}
                        {tech}
                      </span>
                  ...
                  <div className="mt-auto flex justify-between items-center">
                    ...
                    <div className="flex space-x-1">
                      {project.technologies.slice(0, 3).map((tech, idx) => (
                        <span
                          key={idx}
                          className="h-6 w-6 rounded-full bg-purple-900/60 flex items-center justify-center text-purple-200"
                        >
                          {getTechIcon(tech)}
                        </span>
                      ))}
